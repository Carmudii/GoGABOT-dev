...
void World::setupItemDefs()
{
    string listItemDefsFile;
    vector<string> splitedString;
    
    if (!utils::ifExists(gt::item_defs_file_name)) {
        cout << "[ERROR] - Base file and items file was not found on this path." << endl;
        gt::is_exit = true;
        return;
    }
    ...
    short id = 0;
    itemDefs.clear();
    ItemDefinitionStruct def = {};
    ifstream itemDefsFile(gt::item_defs_file_name);
    while (std::getline(itemDefsFile, listItemDefsFile))
    {
        std::istringstream iss(listItemDefsFile);
        if (listItemDefsFile.length() < 2) continue;
        vector<string> metaInfo = utils::split(listItemDefsFile, "\\");
        def.id = atoi(metaInfo[0].c_str());
        def.actionType = atoi(metaInfo[1].c_str());
        def.itemName = metaInfo[2];
        if(def.id != id) {
